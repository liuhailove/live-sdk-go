...
type ConnectOption func(params *ConnectParams)
...
type PLIWriter func(ssrc webrtc.SSRC)
...
func (r *Room) sendSyncState() {
	...
	prevoisSdp := r.engine.subscriber.pc.LocalDescription()
	...
	r.engine.client.SendSyncState(&livekit.SyncState{
		Answer: ToProtoSessionDescription(*prevoisSdp),
...
func unpackStreamID(packed string) (participantId string, trackId string) {
	...
	if len(parts) > 1 {
		return parts[0], packed[len(parts[0])+2:]
	}
	return packed, ""
}
