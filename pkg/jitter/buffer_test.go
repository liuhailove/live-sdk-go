...
func TestJitterBuffer(t *testing.T) {
	...
	pkts := b.Pop(false)
	require.Len(nil, pkts, 5)
	for i, pkt := range pkts {
		require.Equal(nil, uint16(i+1), pkt.SequenceNumber)
	}

	// test push and pop while not empty
	b.Push(testTailPacket(7, 32))

	require.Len(nil, b.Pop(false), 2)
	...
	require.Len(nil, b.Pop(false), 4)
	require.Equal(nil, 0, onPacketDroppedCalled)
	...
	require.Nil(nil, b.Pop(false))
	require.Equal(nil, 1, onPacketDroppedCalled)
	...
	require.Len(nil, b.Pop(false), 4)
	require.Equal(nil, 2, onPacketDroppedCalled)
